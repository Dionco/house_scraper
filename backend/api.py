...
async def update_current_user(
    ...
    if user_update.email:
        ...
        for uid, u in users.items():
            if uid != user_id and u.get("email") == user_update.email:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Email already registered"
                )
    ...
    if user_update.new_password:
        ...
        if not AuthUtils.verify_password(user_update.current_password, user["password_hash"]):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Current password is incorrect"
            )
        
        user["password_hash"] = AuthUtils.hash_password(user_update.new_password)
...
# Helper function to create ProfileResponse with proper last_scraped conversion
def create_profile_response(profile_id: str, profile: dict) -> ProfileResponse:
    """Helper function to create ProfileResponse with proper data type conversion."""
    # Convert last_scraped from ISO string to timestamp if needed
    last_scraped = profile.get("last_scraped")
    if last_scraped and isinstance(last_scraped, str):
        try:
